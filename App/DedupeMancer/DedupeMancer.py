...
import jsonlines
import numpy as np
# faiss imported lazily in perform_min_hash_deduplication to avoid loading on startup
...
# Setup logging: ERRORs only, NO console output
logging.basicConfig(
    level=logging.ERROR,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[]  # No handlers - logging calls will be no-ops
)
...
class Deduplication:
    def __init__(
        self,
        threshold=0.90, # 👈 dropped by 0.06
        num_perm=256,
        seed=0,
        shingle_k=10,
        semantic_threshold=0.89, # 👈 dropped by 0.06
        prefix_bits=16,  
    ):
